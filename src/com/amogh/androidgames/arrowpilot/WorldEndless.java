...
public class WorldEndless { 
	...
	public WorldEndless(WorldListener worldListener, int area){
		...
		this.background[0] = new Background(DEFAULT_LEVEL_OFFSET, WORLD_HEIGHT/2);
		this.background[1] = new Background(DEFAULT_LEVEL_OFFSET*2, WORLD_HEIGHT/2);
	...
	private void generateLevel(int offset){
		...
		if(difficulty == DIFFICULTY_EASY){
			...
			if(area == AREA1){
				int ran = 0;
				do{
					ran = randInt(0,AssetsWorld1.numLevelsAdvanced-1);	
				}while(ran == lastLevel);
				lastLevel = ran;
		...
		for(int i = 0; i < map.size(); i++){
			...
			for(int j = 0; j < array.length; j++){
				...
				if(d == OCTOPUS){
					Monster	m = new Monster(j + offset + Monster.SPIDER_WIDTH/2, 
							WORLD_HEIGHT - i-Monster.SPIDER_HEIGHT/2, 
							area==AREA1?Monster.TYPE_SPIDER:Monster.TYPE_OCTOPUS);
					monsters.add(m);
				}
			}
		}
	...
	private void updateBackground(float deltaTime){
		background[0].update(deltaTime);
		background[1].update(deltaTime);
		if(bgState == 0 && background[0].position.x < 1){
			bgState = 1;
		}
		if(background[0].position.x < 0)
			background[0].position.set(Background.BG_WIDTH, WORLD_HEIGHT/2);
		if(background[1].position.x < 0)
			background[1].position.set(Background.BG_WIDTH, WORLD_HEIGHT/2);
		if(bgState == 1 && background[1].position.x < 1){
			bgState = 0;
		}

		if((background[1].position.x - background[0].position.x) > 16.2f ||
				(background[1].position.x - background[0].position.x) < -16.2f){
			if(background[0].position.x < background[1].position.x)
				background[1].position.x = background[0].position.x + 16;
			if(background[1].position.x < background[0].position.x)
				background[0].position.x = background[1].position.x + 16;
		}
	}

	/*private void leavesManager(float deltaTime){
		leavesTimer += deltaTime;
		//int ran = randInt(0, 3);
		if(leavesTimer > 2){
			Leaf leaf = new Leaf(17, rand.nextFloat()*WORLD_HEIGHT);
			leaves.add(leaf);
			leavesTimer = 0;
		}

		int len = leaves.size();
		Log.d("WorldEndless", "len " + len + " leavesTimer" + leavesTimer);
		for(int i = 0; i < len; i++){
			Leaf l = leaves.get(i);
			l.update(deltaTime);
			if(l.position.x < 0){
