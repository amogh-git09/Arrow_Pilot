...
public class WorldRendererEndless {
	static final int FRUSTUM_WIDTH = 16;
	static final int FRUSTUM_HEIGHT = 10;
	...
	private void renderBackground(){
		...
		if(area == AREA2)
			batcher.beginBatch(AssetsWorld2.bgWorld2);

		batcher.drawSprite(world.background[world.bgState].position.x, 
				world.background[world.bgState].position.y, 
				Background.BG_WIDTH, Background.BG_HEIGHT, 
				area==AREA1?AssetsWorld1.background:
	...
	private void renderMonsters(){
		...
		for(int i = 0; i < len; i++){
			...
			Log.d("WorldRendererEndless", m.position.x + ", " + m.position.y);
			if(m.type == Monster.TYPE_OCTOPUS){
				batcher.drawSprite(m.position.x, m.position.y, 
						Monster.OCTOPUS_WIDTH, Monster.OCTOPUS_HEIGHT, 
						AssetsWorld2.octopus);
			}
			if(m.type == Monster.TYPE_SPIDER){
				batcher.drawSprite(m.position.x, m.position.y, 
						Monster.SPIDER_WIDTH, Monster.SPIDER_HEIGHT, 
						AssetsWorld1.spider);
			}
		}
	}
	...
	/*	public void renderLeaves(){
		...
		for(int i = 0; i < len; i++){
			Leaf l = world.leaves.get(i);
			batcher.drawSprite(l.position.x, l.position.y, l.bounds.width, l.bounds.height, 
					AssetsWorld1.leaves.get(l.type));
		}
	}
	 */	
	/*	public void renderSun(){
		Sun s = world.sun;
		batcher.drawSprite(s.position.x, s.position.y, Sun.SUN_WIDTH, Sun.SUN_HEIGHT, 
				AssetsWorld1.sun);
	}*/
}
